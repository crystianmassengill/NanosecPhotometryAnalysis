...
% Oppenheimer tcp
rigs.oppenheimer.tcp.name = 'Oppenheimer TCP';
rigs.oppenheimer.tcp.mode = '1. Green sensor + 405 movement';
rigs.oppenheimer.tcp.data_channel = 1;
rigs.oppenheimer.tcp.data_channel2 = 1;
rigs.oppenheimer.tcp.opto_channel = 99;
rigs.oppenheimer.tcp.ch1_pulse_ind = 5;
rigs.oppenheimer.tcp.ch2_pulse_ind = 6;
rigs.oppenheimer.tcp.ch1_pulse_thresh = 0.5;
rigs.oppenheimer.tcp.ch2_pulse_thresh = 0.5;
rigs.oppenheimer.tcp.optomode = false;
rigs.oppenheimer.tcp.tone_channel = 99; 

% Oppenheimer tcp
rigs.oppenheimer.optophoto.name = 'Oppenheimer TCP w/ switcher';
rigs.oppenheimer.optophoto.mode = '2. 2x Green sensor + Opto';
rigs.oppenheimer.optophoto.data_channel = 1;
rigs.oppenheimer.optophoto.data_channel2 = 2;
rigs.oppenheimer.optophoto.opto_channel = 6;
rigs.oppenheimer.optophoto.VF_channel = 13;
rigs.oppenheimer.optophoto.ch1_pulse_ind = 12;
rigs.oppenheimer.optophoto.ch2_pulse_ind = 11;
rigs.oppenheimer.optophoto.ch1_pulse_thresh = 0.5;
rigs.oppenheimer.optophoto.ch2_pulse_thresh = 0.5;
rigs.oppenheimer.optophoto.optomode = false;
rigs.oppenheimer.optophoto.tone_channel = 99; 
...
rigs.cleopatra.optophoto.data_channel = 1;
rigs.cleopatra.optophoto.data_channel2 = 3;   %CIM added 2nd channel 230107
rigs.cleopatra.optophoto.opto_channel = 9;
rigs.cleopatra.optophoto.ch1_pulse_ind = 7;
rigs.cleopatra.optophoto.ch2_pulse_ind = 2;
rigs.cleopatra.optophoto.ch1_pulse_thresh = 1;
rigs.cleopatra.optophoto.ch2_pulse_thresh = 1;
...
rigs.cleopatra.tcp.data_channel2 = 1;
rigs.cleopatra.tcp.opto_channel = 99;
rigs.cleopatra.tcp.ch1_pulse_ind = 2;
rigs.cleopatra.tcp.ch2_pulse_ind = 7;
...
if tf
    ...
    blackout_window = 4; % Ignore the first X points within each pulse due to capacitated currents (9 for 2500 Hz)    %CIM changed to 4 to adapt to reduced frequency 230107

    % Channel and frequency data
    freq = 50; % Sampling rate after downsampling (i.e., pulse rate of each channel in Hz)  %CIM changed to 25 to use with switcher 230107
